...
class CFN(object):
    ...
    def select_action(self, obs, is_max):
        sample_action = np.random.uniform(
            ...
            high=np.linspace(self.min_action, self.max_action, self.uniform_action_size, endpoint=False)[:, None] + (self.max_action - self.min_action) / self.uniform_action_size,
            size=(self.uniform_action_size, self.action_dim)
        )
        with torch.no_grad():
    ...
    def train(self, replay_buffer, frame_idx, num_step, batch_size=256, max_episode_steps=50, sample_flow_num=100):
